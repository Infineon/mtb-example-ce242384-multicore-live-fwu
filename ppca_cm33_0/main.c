 ...
 * File Name:   main.c
 *
 * Description: This is the source code for PPCA CM330 core
 *
...
#define PPCA_IMAGE_ALT_VECTOR_OFFSET                                           \
	(PPCA_IS_EVEN_BUILD(IMAGE_BUILD_NUM) ? CYMEM_CM33_0_S_ppca0_code_B_OFFSET  \
										 : CYMEM_CM33_0_S_ppca0_code_A_OFFSET)

/* PPCA Image Alternate Slot Size */
#define PPCA_IMAGE_ALT_SLOT_SIZE                                               \
    (PPCA_IS_EVEN_BUILD(IMAGE_BUILD_NUM) ? CYMEM_CM33_0_S_ppca0_code_B_SIZE    \
                                         : CYMEM_CM33_0_S_ppca0_code_A_SIZE)

/* PPCA SRAM data region used by this core */
#define PPCA_SRAM_DATA_START CYMEM_ppca0_data_START
#define PPCA_SRAM_DATA_SIZE  CYMEM_ppca0_data_SIZE
 ...
 * (PPCA Core 0 -> Main CPU). */
#define PPCA_IPC_CORE_UP_STRUCT  PPCA_IPC_STRUCT0
#define PPCA_IPC_CORE_UP_CH      IPC_CH_CORE0_TO_MAIN

/* IPC channel / structures used to receive a switch request from the Main CPU
 * (Main CPU -> PPCA Core 0). */
#define PPCA_IPC_SWITCH_STRUCT      PPCA_IPC_STRUCT2
#define PPCA_IPC_SWITCH_INTR_STRUCT PPCA_IPC_INTR_STRUCT2
#define PPCA_IPC_SWITCH_CH          IPC_CH_MAIN_TO_CORE0
#define PPCA_IPC_SWITCH_IRQ         ppca_ipc_2_IRQn
...
#endif
volatile ppca_state_t ppca0_state;
...
void PPCA0_PWM_Handler(void);
void PPCA0_IPC_Handler(void);
...
static bool ppca_is_live_update(void)
{
    return ((ppca0_state.start_magic == LIVE_DATA_START_MAGIC) &&
            (ppca0_state.end_magic == LIVE_DATA_END_MAGIC) &&
            (ppca0_state.live_update == LIVE_UPDATE_ACTIVE));
}
...
static void ppca_state_init_default(void)
{
    ppca0_state.start_magic     = LIVE_DATA_START_MAGIC;
    ppca0_state.live_update     = LIVE_UPDATE_INACTIVE;
    ppca0_state.ppca0_pwm_count = 0u;
    ppca0_state.end_magic       = LIVE_DATA_END_MAGIC;
}
...
/*******************************************************************************
 * Function Name: PPCA0_IPC_Handler
 *******************************************************************************
 * Summary:
...
/*******************************************************************************
 * Function Name: PPCA0_PWM_Handler
 ********************************************************************************
 * Summary:
 *  PWM (TCPWM) interrupt handler for PPCA Core 0. Uses the persistent count to
...
void PPCA0_PWM_Handler(void)
{
	Cy_TCPWM_ClearInterrupt(PPCA0_PWM_HW, PPCA0_PWM_NUM, CY_TCPWM_INT_ON_CC0);
	if(ppca0_state.ppca0_pwm_count >= LED_COUNT)
	{
		ppca0_state.ppca0_pwm_count = 0;
		Cy_GPIO_Inv(CYBSP_USER_LED2_PORT, CYBSP_USER_LED2_PIN);
	}
	else
	{
		ppca0_state.ppca0_pwm_count++;
	}
...
********************************************************************************
* Summary:
*  This is the main function for PPCA CM330 CPU. It does...
...
int main(void)
{
    /* On a cold boot, initialize the persistent state. On a live update the
     * state (and the running PWM) is retained across the switch. */
    ...
    {
        /* Configure PPCA0 PWM */
        if (CY_TCPWM_SUCCESS != Cy_TCPWM_PWM_Init(PPCA0_PWM_HW, PPCA0_PWM_NUM, &PPCA0_PWM_config))
        {
            CY_ASSERT(0);
        }

        /* Enable the initialized PWM */
        Cy_TCPWM_PWM_Enable(PPCA0_PWM_HW, PPCA0_PWM_NUM);

        NVIC_SetPriority(PPCA0_PWM_IRQ, PWM_IRQ_PRIORITY);
        NVIC_EnableIRQ(PPCA0_PWM_IRQ);

        /* Start the PWM */
        Cy_TCPWM_TriggerStart_Single(PPCA0_PWM_HW, PPCA0_PWM_NUM);
    }

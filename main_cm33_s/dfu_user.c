...
* ("Software") is owned by Infineon Technologies or one of its
* affiliates ("Infineon") and is protected by and subject to worldwide patent
 ...
 ********************************************************************************
 * Summary:
 *  Registers the serial (external) memory object used by the DFU transport for
 *  external-memory programming.
 *
 * Parameters:
 *  serialMemObj : Pointer to the initialized serial memory object
 *
 * Return:
 *  void
 *
...
/* Read-while-write (RWW) flash pipeline state.
 * The DFU host is assumed to be the only writer of internal flash, so at most
 * one row write may be in flight at a time. Cy_DFU_WriteData() starts a
 * non-blocking erase+program and returns immediately, allowing the transport of
 * the next packet (and all interrupts running from the other flash bank) to
 * overlap the in-progress write. dfu_flash_sync() drains the pending op before
 * the next flash access. The first deferred failure is latched in s_stickyErr
 * so the update aborts deterministically. */
static bool               s_writePending = false;
static uint32_t           s_pendingAddr  = 0u;
static cy_en_dfu_status_t s_stickyErr    = CY_DFU_SUCCESS;

static cy_en_dfu_status_t dfu_flash_sync(void);

#if defined CY_DUAL_FLASH_S_SBUS_BASE
/* DFU stages the new image into the alternate (staging) flash bank only; the
 * running bank must never be written during an update. */
#define DFU_STAGING_BANK_BASE   (CY_DUAL_FLASH_S_SBUS_BASE)
#define DFU_STAGING_BANK_SIZE   (0x00040000UL)
#endif /* defined CY_DUAL_FLASH_S_SBUS_BASE */
...
static bool AddressValid(uint32_t address, cy_stc_dfu_params_t *params)
{
    ...
    ((CY_EXT_NVM1_BASE <= address) && (address < (CY_EXT_NVM1_BASE + CY_EXT_NVM1_SIZE)));
        ...
        #else
            #if defined CY_DUAL_FLASH_S_SBUS_BASE
                addrValid = (DFU_STAGING_BANK_BASE <= address) &&
                            (address < (DFU_STAGING_BANK_BASE + DFU_STAGING_BANK_SIZE));
            #else
...
/*******************************************************************************
* Function Name: dfu_flash_sync
****************************************************************************//**
*
* Drains any in-flight non-blocking flash write started by Cy_DFU_WriteData().
* Idempotent: safe to call when nothing is pending. After the op completes the
* flash cache is cleared so that subsequent reads of the just-written rows are
* coherent. The first failure observed is latched into s_stickyErr so that the
* whole update aborts deterministically (a partially written image is invalid).
*
* \return The sticky DFU status (CY_DFU_SUCCESS until a flash op has failed).
*
*******************************************************************************/
static cy_en_dfu_status_t dfu_flash_sync(void)
{
    if (s_writePending)
    {
        ...
        do
        {
            fstatus = Cy_Flash_IsOperationComplete();
        } while (fstatus == CY_FLASH_DRV_OPCODE_BUSY);

        s_writePending = false;

        /* Make reads of the freshly written rows coherent. On MXS40FLASHC the
         * PDL flash driver clears the flash data cache/buffer automatically, so
         * only the CPU-subsystem instruction cache must be invalidated here
         * (the M33 reads flash through CBUS/ICACHE). Mirrors the BSP HAL's
         * _mtb_hal_flash_clear_cache(). Cy_SysLib_ClearFlashCacheAndBuffer() is
         * not available on this device (it is gated behind CY_IP_M4CPUSS). */
        ...
        if ((fstatus != CY_FLASH_DRV_SUCCESS) && (s_stickyErr == CY_DFU_SUCCESS))
        {
            s_stickyErr = CY_DFU_ERROR_DATA;
            CY_DFU_LOG_ERR("Deferred flash write failed at 0x%X, fstatus 0x%X",
                           (unsigned int)s_pendingAddr, (unsigned int)fstatus);
        }
    }
...
/*******************************************************************************
* Function Name: app_dfu_flash_wait_complete
****************************************************************************//**
*
* Public wrapper used by the application to force-complete any pending DFU
* flash write before reading the staged image (e.g. before validating or
* switching to the new image). Must be called after the last row is written and
* before any read of the staging bank.
*
* \return CY_DFU_SUCCESS if all flash writes completed successfully, otherwise
*         the latched error status.
*
*******************************************************************************/
cy_en_dfu_status_t app_dfu_flash_wait_complete(void)
{
    return dfu_flash_sync();
}


/*******************************************************************************
* Function Name: app_dfu_erase_staging_header
****************************************************************************//**
*
* Recovery action taken when the freshly downloaded image fails validation.
* Starts a NON-BLOCKING erase of only the first row of the staging bank, which
* holds the image magic and version. Erasing that row invalidates the staged
* image so the next download starts clean. Uses the same RWW gate as writes:
* the erase is drained by dfu_flash_sync() at the start of the next
* Cy_DFU_WriteData()/Cy_DFU_ReadData() once DFU is restarted. The sticky error
* and pending state are reset so the next attempt is not pre-aborted.
*
* \return CY_DFU_SUCCESS if the erase started, otherwise an error status.
*
*******************************************************************************/
cy_en_dfu_status_t app_dfu_erase_staging_header(void)
{
#if defined CY_DUAL_FLASH_S_SBUS_BASE
    cy_en_flashdrv_status_t fstatus;

    /* Make sure no previous op is in flight before issuing the erase. */
    (void)dfu_flash_sync();

    /* This is a recovery/abort path: clear the latched error and pending flag
     * so the next download attempt starts from a clean state. */
    s_stickyErr    = CY_DFU_SUCCESS;
    s_writePending = false;

    /* Erase only the first row (image magic + version live here). Non-blocking. */
    fstatus = Cy_Flash_StartEraseRow(DFU_STAGING_BANK_BASE);
    if (fstatus == CY_FLASH_DRV_OPERATION_STARTED)
    {
        s_pendingAddr  = DFU_STAGING_BANK_BASE;
        s_writePending = true;
        return CY_DFU_SUCCESS;
    }

    CY_DFU_LOG_ERR("Flash StartEraseRow failed: status 0x%X", (unsigned int)fstatus);
    return CY_DFU_ERROR_DATA;
#else
    return CY_DFU_ERROR_UNKNOWN;
#endif /* defined CY_DUAL_FLASH_S_SBUS_BASE */
}
                                               ...
                                               cy_stc_dfu_params_t *params)
{
    ...
    /* Complete the previously started (pipelined) row write before starting a
     * new one. This overlaps the prior flash write with the transport of the
     * current packet. If any prior write failed, abort the whole update. */
    status = dfu_flash_sync();
    if (status != CY_DFU_SUCCESS)
    {
        return status;
    }
...
CY_MISRA_DEVIATE_LINE('MISRA C-2012 Rule 11.3','Casting uint8_t* to uint32_t* is safe as input address is always valid and aligned.');
                /* Start a non-blocking erase+program (read-while-write). No
                 * critical section is used here: the design keeps interrupts
                 * serviceable during flash writes (the CPU and all ISRs run
                 * from the other flash bank), and the DFU host is the sole
                 * writer of internal flash, so the start sequence cannot race.
                 * The op is drained at the start of the next Cy_DFU_WriteData()
                 * / Cy_DFU_ReadData(), or via app_dfu_flash_wait_complete(). */
                cy_en_flashdrv_status_t startStatus =
                    Cy_Flash_StartWrite(address, (const uint32_t*)params->dataBuffer);

                if (startStatus == CY_FLASH_DRV_OPERATION_STARTED)
                {
                    s_pendingAddr  = address;
                    s_writePending = true;
                }
                else
                {
                    status = CY_DFU_ERROR_DATA;
                    CY_DFU_LOG_ERR("Flash StartWrite failed: status 0x%X ", (unsigned int)startStatus);
                }
                (void)fstatus;
            #endif /* defined CY_IP_MXS40SSRSS && defined COMPONENT_NON_SECURE_DEVICE */
        #endif /* CY_IP_M7CPUSS */
                                              ...
                                              cy_stc_dfu_params_t *params)
{
    ...
    /* Ensure any pipelined write has completed (and the flash cache is cleared)
     * before reading back the staging bank, so reads are coherent. Abort if a
     * prior write failed. */
    status = dfu_flash_sync();
    if (status != CY_DFU_SUCCESS)
    {
